...
function renderAdminItems() {
  ...
  main.innerHTML = `
    ...
    <div class="card">
      <div class="field" style="display:flex; gap:10px; align-items:flex-end;">
        ...
        <button class="btn btn-primary" id="addItemMasterBtn">Add</button>
      </div>
    </div>
    <div class="card">
      <table class="admin-table">
  ...
  document.getElementById("addItemMasterBtn").onclick = async () => {
    ...
    if (!name) return;
    await addDoc(collection(db, "itemMaster"), { name });
...
function drawItemMasterRows() {
  ...
  tbody.innerHTML = itemMasterList.map(m => `
    <tr><td>${esc(m.name)}</td><td><button class="btn btn-ghost rm-item" data-id="${m.id}">Remove</button></td></tr>
  `).join("") || `<tr><td colspan="2" style="color:var(--ink-faint);">No items yet.</td></tr>`;
